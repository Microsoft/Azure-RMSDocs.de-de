...
Use the following information to see what’s new or changed for a supported release. Die neueste Version ist zuerst aufgeführt. 

> [!NOTE]
> Minor fixes are not listed so if you experience a problem with the SDK, we recommend that you check whether it is fixed with the latest GA release. Wenn das Problem weiterhin besteht, überprüfen Sie die aktuelle Vorschauversion.
>  
