---
title: Bereitstellen der Azure Information Protection-Überprüfung - Vorgängerversionen
description: Anweisungen zur Bereitstellung für die Versionen der Azure Information Protection-Überprüfung, die älter als die aktuelle allgemein verfügbare Version.
author: cabailey
ms.author: cabailey
manager: barbkess
ms.date: 07/02/2019
...
ms.suite: ems
ms.openlocfilehash: 56b2c28b15ce86237a0c8bb14f2a17739cc44678
ms.sourcegitcommit: 2400948db375d489e8f47a041695ca94124c7506
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 07/01/2019
ms.locfileid: "67488325"
---
...
> 
> Wenn Sie Anweisungen zur Bereitstellung für die aktuelle Version des Scanners, die Konfiguration von Azure-Portal enthält suchen, finden Sie unter [Bereitstellen der Azure Information Protection-Überprüfung zum automatischen klassifizieren und Schützen von Dateien](deploy-aip-scanner.md).
...
- Standorte und -Bibliotheken für SharePoint Server 2019 über SharePoint Server 2013. SharePoint 2010 wird außerdem für Kunden unterstützt, die über [erweiterten Support für diese Version von SharePoint](https://support.microsoft.com/lifecycle/search?alpha=SharePoint%20Server%202010) verfügen.
...
Verwenden Sie das Cmdlet **Add-AIPScannerRepository**, um die Datenspeicher anzugeben, die von der Azure Information Protection-Überprüfung gescannt werden sollen. Sie können lokale Ordner, UNC-Pfade und SharePoint Server-URLs für SharePoint-Websites und -Bibliotheken angeben. 

Unterstützte Versionen für SharePoint: 2019 für SharePoint-Server, SharePoint Server 2016 und SharePoint Server 2013. SharePoint Server 2010 wird außerdem für Kunden unterstützt, die über [erweiterten Support für diese Version von SharePoint](https://support.microsoft.com/lifecycle/search?alpha=SharePoint%20Server%202010) verfügen.
...
1. Fügen Sie in der PowerShell-Sitzung über denselben Windows Server-Computer den ersten Datenspeicher hinzu, indem Sie den folgenden Befehl ausführen:
    ...
    Weitere Beispiele, verwenden Sie den PowerShell-Hilfebefehl `Get-Help Add-AIPScannerRepository -examples` für dieses Cmdlet.
...
Sie können erzwingen, dass die Überprüfung erneut überprüfen Sie alle Dateien mit **Start-AIPScan** mit der *zurücksetzen* Parameter. Die Überprüfung muss konfiguriert werden, für einen manuellen Zeitplan müssen die *Zeitplan* Parameter festgelegt werden, um **manuelle** mit **Set-AIPScannerConfiguration**.
...
- Identifizieren aller benutzerdefinierten Bedingungen und bekannten vertraulichen Informationstypen:
    ...
    Der folgende Schnellstart verwendet dieser Konfiguration, obwohl es für die aktuelle Version des Scanners ist: [Schnellstart: Bestimmen vertraulicher Informationen](quickstart-findsensitiveinfo.md).
...
- Der ausgewählte Protokolliergrad
    ...
    Sie können für die Überprüfungsberichte zwischen **Debuggen**, **Info**, **Fehler** und **Aus** wählen. **Aus** führt zur besten Leistung; **Debuggen** verlangsamt die Überprüfung erheblich und sollte nur zur Problembehebung verwendet werden. Weitere Informationen finden Sie unter den *ReportLevel* Parameter für das Cmdlet Set-AIPScannerConfiguration mit `Get-Help Set-AIPScannerConfiguration -detailed`.

- Die Dateien selbst:
    
    - Mit Ausnahme von Excel-Dateien werden die Office-Dateien schneller überprüfen als PDF-Dateien.
...
> [!NOTE]
> Viele dieser Cmdlets sind jetzt veraltet, in der aktuellen Version des Scanners ein, und die Onlinehilfe für die Cmdlets für die Überprüfung gibt, diese Änderung. Cmdlet-Hilfe für zuvor die integrierte Version 1.48.204.0 der Überprüfung, verwenden Sie für `Get-Help <cmdlet name>` in Ihre PowerShell-Sitzung den Befehl.
