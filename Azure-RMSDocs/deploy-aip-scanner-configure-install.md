...
1. Konfigurieren Sie für diese Erstkonfiguration die folgenden Einstellungen, und wählen Sie dann **Speichern** aus, aber schließen Sie den Bereich nicht.
    ...
    |**DLP-Richtlinie** | Wenn Sie eine DLP-Richtlinie (Microsoft 365 Data Loss Prevention) verwenden, legen Sie **DLP-Regeln aktivieren** **auf ein fest.** Weitere Informationen finden Sie unter [Verwenden einer DLP-Richtlinie](#use-a-dlp-policy-public-preview). |
    ...
    |**Konfigurieren von Datei Einstellungen**     | - **"Datum geändert", "zuletzt geändert" und "geändert von**" beibehalten: behalten Sie die Standardeinstellung **bei** . <br />- **Zu überprüfende Dateitypen**: Standard Dateitypen für **Ausschluss** beibehalten <br />- **Standard Besitzer**: behalten Sie die Standardeinstellung **Scanner-Konto** bei.  <br /> - **Repository-Besitzer festlegen**: Verwenden Sie diese Option nur, wenn [Sie eine DLP-Richtlinie verwenden](#use-a-dlp-policy-public-preview). |
    | | |
...
## <a name="use-a-dlp-policy-public-preview"></a>Verwenden einer DLP-Richtlinie (öffentliche Vorschau)

Durch die Verwendung Microsoft 365 einer DLP-Richtlinie (Data Loss Prevention, Verhinderung von Datenverlust) kann der Scanner potenzielle Datenverluste erkennen, indem er DLP-Regeln mit in Dateifreigaben und SharePoint Server gespeicherten Dateien abschließt

- **Aktivieren Sie DLP-Regeln im Inhalts Überprüfungs Auftrag** , um das verfügbar machen von Dateien zu verringern, die ihren DLP-Richtlinien entsprechen. Wenn Ihre DLP-Regeln aktiviert sind, kann der Scanner den Dateizugriff nur auf Daten Besitzer reduzieren oder die Gefährdung von Netzwerk weiten Gruppen, wie z. b. **allen** **Benutzern, authentifizierten Benutzern** oder **Domänen Benutzern**, verringern. 

- Legen Sie **in der Microsoft 365**, die das Admin Center bezeichnet, fest, ob Sie nur ihre DLP-Richtlinie testen möchten oder ob die Regeln erzwungen werden sollen und ihre Dateiberechtigungen gemäß diesen Regeln geändert werden sollen. Weitere Informationen finden Sie unter Aktivieren [einer DLP-Richtlinie](/microsoft-365/compliance/create-test-tune-dlp-policy#turn-on-a-dlp-policy).
...
1. Navigieren Sie im Azure-Portal zum Auftrag für die Inhalts Überprüfung. Weitere Informationen finden Sie unter [Erstellen eines Inhalts Scan Auftrags](#create-a-content-scan-job).

1. Legen Sie unter **DLP-Richtlinie** die Option **DLP-Regeln aktivieren** **auf ein** fest.

    > [!IMPORTANT]
    > Legen Sie **DLP-Regeln aktivieren** nicht auf **on** fest, es sei denn, in Microsoft 365 ist eine DLP-Richtlinie konfiguriert. 
    >
    >Wenn diese Funktion ohne eine DLP-Richtlinie aktiviert wird, generiert der Scanner Fehler.
1. Optionale Legen Sie unter **Datei Einstellungen konfigurieren** den **Besitzer des Set-Repository** **auf ein** fest, und definieren Sie einen bestimmten Benutzer als Repository-Besitzer.  

    Diese Option ermöglicht es dem Scanner, das verfügbar machen von Dateien in diesem Repository, die der DLP-Richtlinie entsprechen, mit dem definierten Repository-Besitzer zu verringern.

### <a name="dlp-policies-and-make-private-actions"></a>DLP-Richtlinien und *Erstellen privater* Aktionen

Wenn Sie eine DLP-Richtlinie mit einer Aktion " *private erstellen* " verwenden und auch die Überprüfung verwenden, um Ihre Dateien automatisch zu bezeichnen, empfiehlt es sich, auch die erweiterte Einstellung " [**usecopyandkonservientfsowner**](rms-client/clientv2-admin-guide-customizations.md#preserve-ntfs-owners-during-labeling-public-preview) " des Unified Label-Clients zu definieren. 

Mit dieser Einstellung wird sichergestellt, dass die ursprünglichen Besitzer den Zugriff auf Ihre Dateien behalten.

Weitere Informationen finden Sie unter [Erstellen eines Inhalts Überprüfungs Auftrags](#create-a-content-scan-job) und  [Anwenden einer Vertraulichkeits Bezeichnung auf Inhalt automatisch](/microsoft-365/compliance/apply-sensitivity-label-automatically) in der Microsoft 365-Dokumentation. 
