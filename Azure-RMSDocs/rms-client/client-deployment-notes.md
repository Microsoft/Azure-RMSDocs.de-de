...
3. **Der Azure Rights Management-Ermittlungsdienst**: Der RMS-Client stellt eine Verbindung zu **`https://discover.aadrm.com`** her, wo der Benutzer zur Authentifizierung aufgefordert wird.
    ...
    *\<yourtenanturl @ no__t-2* weist das folgende Format auf: **{GUID}. RMS. [Region]. aadrm. com**. Sie finden diesen Wert, indem Sie den Wert von **rightionmanagementserviceid** ermitteln, wenn Sie das Cmdlet [Get-aipserviceconfiguration](/powershell/module/aipservice/get-aipserviceconfiguration) ausführen.
